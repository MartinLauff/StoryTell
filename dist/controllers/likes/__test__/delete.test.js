...
it('has a route handler listening to /api/upvotes/:id/unlike for like requests', async () => {
    ...
    const response = await (0, supertest_1.default)(app_1.default).put(`/api/upvotes/${id}/unlike`).send({});
    expect(response.status).not.toEqual(404);
});
it('can only be accessed if the user is signed in', async () => {
    const id = new mongoose_1.default.Types.ObjectId().toHexString();
    await (0, supertest_1.default)(app_1.default).put(`/api/upvotes/${id}/unlike`).send({}).expect(401);
});
...
it('deletes a like', async () => {
    ...
    await (0, supertest_1.default)(app_1.default)
        ...
        .send()
        .expect(200);
    const response3 = await (0, supertest_1.default)(app_1.default)
        .get(`/api/posts/${response.body._id}`)
        .set('Authorization', `Bearer ${token}`)
        .send();
