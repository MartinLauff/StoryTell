...
it('can fetch a list of posts', async () => {
    ...
    await supertest_1.default(app_1.default).post('/api/posts').set('Cookie', cookie).send({
        topic: 'extras',
        title: 'My first client',
        content: 'Lorem ipsum',
        coverImage: 'img',
    });
    const res = await supertest_1.default(app_1.default).post('/api/posts').set('Cookie', cookie).send({
        topic: 'extras',
        title: 'My first client',
        content: 'Lorem ipsum',
        coverImage: 'img',
