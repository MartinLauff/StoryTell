...
it('has a route handler listening to /api/upvotes/:id/like for like requests', async () => {
    ...
    const response = await supertest_1.default(app_1.default).put(`/api/upvotes/${id}/like`).send({});
    expect(response.status).not.toEqual(404);
});
it('can only be accessed if the user is signed in', async () => {
    const id = new mongoose_1.default.Types.ObjectId().toHexString();
    await supertest_1.default(app_1.default).put(`/api/upvotes/${id}/like`).send({}).expect(401);
});
