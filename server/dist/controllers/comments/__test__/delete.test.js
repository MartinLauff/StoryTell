...
it('has a route handler listening to /api/comments/:id for like requests', async () => {
    ...
    const response = await (0, supertest_1.default)(app_1.default).delete(`/api/comments/${id}`).send({});
    expect(response.status).not.toEqual(404);
});
it('can only be accessed if the user is signed in', async () => {
    const id = new mongoose_1.default.Types.ObjectId().toHexString();
    await (0, supertest_1.default)(app_1.default).delete(`/api/comments/${id}`).send({}).expect(401);
});
